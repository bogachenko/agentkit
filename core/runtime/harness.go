...
// Harness owns the full deterministic run loop without embedding semantic intent detection.
type Harness struct {
	Model        port.Model
...
// Run executes one bounded agent run through model, route validation, optional tool execution, and ledger.
func (h Harness) Run(ctx context.Context, command RunCommand) (RunResult, error) {
	...
	if err := h.publish(ctx, port.EventTypeStarted, command, map[string]any{
	...
	for stepIndex := 0; stepIndex < command.MaxSteps; stepIndex++ {
		response, err := h.Model.Generate(ctx, llm.Request{
			System:         command.System,
			Messages:       messages,
			RuntimeContext: command.RuntimeContext,
		})
		if err != nil {
			return h.failedResult(ctx, command, ledger, Failure{
				Code:    FailureCodeModelFailed,
				Message: err.Error(),
			}, stepIndex)
		}
		...
		decisionResult, err := orchestrator.HandleDecision(ctx, DecisionCommand{
		...
		case RouteKindRespond, RouteKindComplete:
			...
			if err := h.publish(ctx, port.EventTypeCompleted, command, map[string]any{
		...
		case RouteKindRequireApproval:
			failure := Failure{
				...
				Message: fmt.Sprintf("tool %q requires approval", string(decisionResult.Decision.ToolName)),
			}

			if err := h.publish(ctx, port.EventTypeBlocked, command, map[string]any{
				"status":       string(RunStatusBlocked),
				"failure_code": string(failure.Code),
				"tool_name":    string(decisionResult.Decision.ToolName),
			}); err != nil {
				return RunResult{}, err
			}

			return h.blockedResult(command, ledger, RouteDecision{
				Kind:    RouteKindBlocked,
				Reason:  "Runtime requires user approval before tool execution.",
				Failure: &failure,
			}, stepIndex+1)
		...
		default:
			return h.failedResult(ctx, command, ledger, Failure{
				Code:    FailureCodeInvalidDecision,
				Message: fmt.Sprintf("unsupported route kind %q", string(decisionResult.Decision.Kind)),
			}, stepIndex+1)
		}
	}
	...
	return h.failedResult(ctx, command, ledger, Failure{
		...
		Message: "run reached max steps without terminal decision",
	}, command.MaxSteps)
}

// Dependency validation prevents silent partial harness execution.
...
// executeTool runs only an explicit validated tool decision and records call/result facts.
func (h Harness) executeTool(ctx context.Context, command RunCommand, ledger *Ledger, decision RouteDecision) (llm.Message, error) {
	...
	if err := ledger.Append(NewToolCallEntry(
		LedgerEntryID(h.IDGenerator.NewID()),
		command.RunID,
		call,
		h.Clock.Now(),
	)); err != nil {
	...
	if err := h.publish(ctx, port.EventTypeStep, command, map[string]any{
		"status":    "tool_completed",
		"tool_name": string(result.Name),
	}); err != nil {
		return llm.Message{}, err
	}
	...
	return llm.NewMessage(
		llm.RoleTool,
		llm.FunctionResponsePart(string(result.Name), result.Output),
	), nil
}

// blockedResult keeps blocked terminal output structurally consistent.
...
// failedResult publishes and returns deterministic failed run output.
func (h Harness) failedResult(ctx context.Context, command RunCommand, ledger *Ledger, failure Failure, stepsCompleted int) (RunResult, error) {
	...
	if err := h.publish(ctx, port.EventTypeFailed, command, map[string]any{
